...
- **LLM Engine**: Google Gemini 3 Flash (`google-genai` SDK)
- **Core Libs**: Pydantic v2, SQLAlchemy 2.0 (Async), `uv` (Package Manager)
- **Database**: PostgreSQL (Independent Schema / Shared Data Source)
...
UFC 플랫폼은 안정적이고 확장성 있는 마이크로서비스 형태의 아키텍처로 구성되어 있습니다.

```mermaid
graph TD
    %% 프론트엔드
    Client(["웹 브라우저"]) -->|"HTTPS 요청"| FE["Frontend<br/>Next.js 15 / Vercel"]
    ...
    subgraph AI Module
        BE -->|"Trigger (매일)"| AI["AI Analysis<br/>FastAPI / Hugging Face"]
        AI <-->|"GenAI SDK"| Gemini[("Google Gemini API")]
    end
    
    %% 데이터 저장소
    BE <-->|"JPA / QueryDSL"| DB[("PostgreSQL<br/>Supabase")]
    BE <-->|"Spring Cache"| Redis[("Redis<br/>Upstash")]
    AI <-->|"SQLAlchemy (Async)"| DB
```
...
**3. 데이터 수집 회복 탄력성 및 보간 (API Resilience & Fallback)**
- **도입 배경**: GitHub API 횟수 초과나 통신 장애로 수집이 실패할 경우, 프론트엔드 차트가 왜곡되거나 지표가 0으로 표기되는 치명적인 문제가 있었습니다.
- **해결 및 효과**: 수집 실패 시 1차적으로 **최대 3회 재시도(Retry)**를 수행하여 순간적인 장애를 극복합니다. 모든 시도가 실패했을 시점에는 직전 시간대 데이터를 끌어와 **빈 값을 보간(Fallback)** 처리함으로써 어떠한 통신 장애 속에서도 프론트엔드 차트의 시각적 무결성을 안전하게 보장합니다.
...
- **해결 및 효과**: 최신 `google-genai` SDK를 활용하여 **Gemini 3 Flash** 기반 해설 시스템을 구축했습니다. 매일 기술들의 지표를 분석해 스포츠 중계 톤의 한글 해설을 생성하고, 허깅페이스 스페이스를 통해 안정적으로 서빙합니다.

**13. AI 분석 무결성 보장 (Data Completeness)**
...
UFCLLM_DB_HOST=your_db_host
UFCLLM_DB_USER=your_db_user
UFCLLM_DB_PASS=your_db_pass
UFCLLM_DB_BASE=your_db_name
```
...
- **AI Analysis (HF)**: [Hugging Face Spaces - UFC LLM](https://huggingface.co/spaces/nx2803/UFC_LLM)
